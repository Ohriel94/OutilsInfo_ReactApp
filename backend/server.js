...
app.get('/recupererAdministrateur/:administrateurID', async (req, res) => {
	...
	try {
		const response = await adminsDM.recupererAdministrateurParId(
			administrateurID
		);
		res.send(response);
	} catch (e) {
...
app.post('/creerAppareil', async (req, res) => {
	...
	try {
		const response = await appareilsDM.creerAppareil(
			serNum,
			mar,
			mod,
			dateAcqu,
...
app.get('/recupererAppareil/serialNumber?:serialNumber', async (req, res) => {
	...
	try {
		const response = await appareilsDM.recupererAppareilParSerialNumber(
			serialNumber
		);
		res.send(response);
	} catch (e) {
...
app.get('/recupererOrdinateur/:ordinateurID', async (req, res) => {
	...
	try {
		const response = await ordinateursDM.recupererOrdinateurParId(
			ordinateurID
		);
		res.send(response);
	} catch (e) {
